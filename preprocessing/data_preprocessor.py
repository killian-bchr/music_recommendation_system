...
class DataPreprocessor:
    ...
    def vectorize_artists(artists_df: DataFrame) -> DataFrame:
        tags_encoded = DataPreprocessor.multihot_encode(
            artists_df[ArtistColumns.TAGS],
            "tag"
        )
        similar_encoded = DataPreprocessor.multihot_encode(
            artists_df[ArtistColumns.SIMILAR_ARTISTS],
            "similar"
        )
    ...
    def apply_svd_reduction(df: DataFrame, n_components: int = 80) -> DataFrame:
        ...
        svd = TruncatedSVD(n_components=min(n_components, df.shape[1] - 1), random_state=42)
    ...
    @staticmethod
    def aggregate_artist_features(tracks_df: DataFrame, artists_df: DataFrame) -> DataFrame:
        ...
        for agg_type in ['mean', 'max', 'std']:
            artist_feature_names.extend([f"artist_{agg_type}_{col}" for col in artist_features.columns])
    ...
    def vectorize_tracks(tracks_df: DataFrame, artists_df: DataFrame) -> DataFrame:
        ...
        tracks_processed[TrackColumns.RELEASE_DAYS] = DataPreprocessor.encode_date_column(
            tracks_df[TrackColumns.RELEASE_DATE]
        )
        ...
        tracks_processed = pd.concat(
            [tracks_processed.drop(columns=[TrackColumns.ARTIST_IDS]), artist_features_matrix],
            axis=1
        )
